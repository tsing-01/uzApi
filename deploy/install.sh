#!/bin/bash
#
# uzApi Installation Script
# uzApi 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/uzapi/main/deploy/install.sh | bash
#
...
# Configuration
GITHUB_REPO="Wei-Shaw/uzapi"
INSTALL_DIR="/opt/uzapi"
SERVICE_NAME="uzapi"
SERVICE_USER="uzapi"
CONFIG_DIR="/etc/uzapi"
...
declare -A MSG_ZH=(
    ...
    # Completion
    ["install_complete"]="uzApi 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 uzApi 服务："
    ...
    ["not_installed"]="uzApi 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 uzApi。"
    ...
    ["uninstall_complete"]="uzApi 已卸载"

    # Help
    ["usage"]="用法"
    ["cmd_none"]="(无参数)"
    ["cmd_install"]="安装 uzApi"
    ["cmd_upgrade"]="升级到最新版本"
    ["cmd_uninstall"]="卸载 uzApi"
    ...
    ["server_config_desc"]="配置 uzApi 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="uzApi Installation Script"
    ...
    # Completion
    ["install_complete"]="uzApi installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start uzApi service:"
    ...
    ["not_installed"]="uzApi is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove uzApi from your system."
    ...
    ["uninstall_complete"]="uzApi has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install uzApi"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove uzApi"
    ...
    ["server_config_desc"]="Configure uzApi server listen address"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/uzapi" ]; then
        # Use grep -E for better compatibility (works on macOS and Linux)
        "$INSTALL_DIR/uzapi" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    ...
    local archive_name="uzapi_${version_num}_${OS}_${ARCH}.tar.gz"
    ...
    # Copy binary
    cp "$TEMP_DIR/uzapi" "$INSTALL_DIR/uzapi"
    chmod +x "$INSTALL_DIR/uzapi"
    ...
    print_success "$(msg 'binary_installed') $INSTALL_DIR/uzapi"
}
...
install_service() {
    ...
    cat > /etc/systemd/system/uzapi.service << EOF
[Unit]
Description=uzApi - AI API Gateway Platform
Documentation=https://github.com/uzapi
...
User=uzapi
Group=uzapi
WorkingDirectory=/opt/uzapi
ExecStart=/opt/uzapi/uzapi
Restart=always
RestartSec=5
StandardOutput=journal
StandardError=journal
SyslogIdentifier=uzapi
...
enable_autostart() {
    ...
    if systemctl enable uzapi 2>/dev/null; then
...
print_completion() {
    ...
    echo "=============================================="
    echo ""
    echo "  $(msg 'cmd_status'):   sudo systemctl status uzapi"
    echo "  $(msg 'cmd_logs'):     sudo journalctl -u uzapi -f"
    echo "  $(msg 'cmd_restart'):  sudo systemctl restart uzapi"
    echo "  $(msg 'cmd_stop'):     sudo systemctl stop uzapi"
    echo ""
    echo "=============================================="
}
...
upgrade() {
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/uzapi" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    print_info "$(msg 'current_version'): $CURRENT_VERSION"

    # Stop service
    if systemctl is-active --quiet uzapi; then
        print_info "$(msg 'stopping_service')"
        systemctl stop uzapi
    fi

    # Backup current binary
    cp "$INSTALL_DIR/uzapi" "$INSTALL_DIR/uzapi.backup"
    print_info "$(msg 'backup_created'): $INSTALL_DIR/uzapi.backup"
    ...
    # Set permissions
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/uzapi"

    # Start service
    print_info "$(msg 'starting_service')"
    systemctl start uzapi

    print_success "$(msg 'upgrade_complete')"
}

# Install specific version (for upgrade or rollback)
# Requires: uzApi must already be installed
install_version() {
    ...
    if [ -f "$INSTALL_DIR/uzapi" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="uzapi.backup.${current_version}"
        else
            backup_name="uzapi.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp "$INSTALL_DIR/uzapi" "$INSTALL_DIR/$backup_name"
...
uninstall() {
    ...
    print_info "$(msg 'stopping_service')"
    systemctl stop uzapi 2>/dev/null || true
    systemctl disable uzapi 2>/dev/null || true

    print_info "$(msg 'removing_files')"
    rm -f /etc/systemd/system/uzapi.service
    systemctl daemon-reload
